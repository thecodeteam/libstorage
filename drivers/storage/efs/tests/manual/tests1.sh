...
#note the behavior of returning an empty string is different from
#other storage drivers. most print the name of the volume being unmounted
...
#Disabled TEST9 and TEST10 because detach on EFS doesnt really do anything
#TEST9=$(sudo rexray volume detach $FIRST_VOLUME | grep "$FIRST_VOLUME" | awk '{print $3}')
#if [ "$TEST9" == "available" ]; then
#  printf "9:rexray volume detach:PASS\n" >> /tmp/output.txt
#else
#  printf "9:rexray volume detach:FAILED\n" >> /tmp/output.txt
#fi
#if [ "$DEBUG" == "true" ]; then
#  printf "#%s\n" "$TEST9" >> /tmp/output.txt
#fi
printf "9:rexray volume detach:DISABLED\n" >> /tmp/output.txt

#TEST10=$(sudo rexray volume ls $FIRST_VOLUME --format=jsonp | jq '.[0].attachments[0].instanceID.driver' | sed -e 's|["'\'']||g')
#if [ "$TEST10" == "null" ]; then
#  printf "10:rexray volume ls jsonp:PASS\n" >> /tmp/output.txt
#else
#  printf "10:rexray volume ls jsonp:FAILED\n" >> /tmp/output.txt
#fi
#if [ "$DEBUG" == "true" ]; then
#  printf "#%s\n" "$TEST10" >> /tmp/output.txt
#fi
printf "10:rexray volume ls jsonp:DISABLED\n" >> /tmp/output.txt
