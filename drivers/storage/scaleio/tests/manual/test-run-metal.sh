...
NODE1_FQDN=${NODE1_FQDN:-$1}
NODE2_FQDN=${NODE2_FQDN:-$2}
NODE3_FQDN=${NODE3_FQDN:-$3}
#NODE4_FQDN=${NODE4_FQDN:-$5}
GIT_LIBSTORAGE_COMMIT_ID=${GIT_LIBSTORAGE_COMMIT_ID:-$4}
GIT_REXRAY_COMMIT_ID=${GIT_REXRAY_COMMIT_ID:-$5}
...
usage() {
  echo "Usage: ${0} node1_fqdn node2_fqdn node3_fqdn, node4_fqdn git-libstorage-commit git-rexray-commit"
  echo ""
...
ssh $LINUX_USER@$NODE1_FQDN "sudo rm -rf /tmp/finished.txt"
...
ssh $LINUX_USER@$NODE2_FQDN "sudo rm -rf /tmp/finished.txt"
...
ssh $LINUX_USER@$NODE3_FQDN "sudo rm -rf /tmp/finished.txt"
...
#ssh $LINUX_USER@$NODE4_FQDN "sudo rm -rf /tmp/finished.txt"
...
ssh $LINUX_USER@$NODE1_FQDN "bash --login -c '/tmp/tests1.sh mytests $GIT_LIBSTORAGE_COMMIT_ID $GIT_REXRAY_COMMIT_ID' 2>&1" &
ssh $LINUX_USER@$NODE2_FQDN "bash --login -c '/tmp/tests2a.sh mytests $GIT_LIBSTORAGE_COMMIT_ID $GIT_REXRAY_COMMIT_ID' 2>&1" &
ssh $LINUX_USER@$NODE3_FQDN "bash --login -c '/tmp/tests3.sh mytests $GIT_LIBSTORAGE_COMMIT_ID $GIT_REXRAY_COMMIT_ID' 2>&1" &
#ssh $LINUX_USER@$NODE4_FQDN "bash --login -c '/tmp/tests4.sh mytests $GIT_LIBSTORAGE_COMMIT_ID $GIT_REXRAY_COMMIT_ID' 2>&1" &
...
ssh $LINUX_USER@$NODE2_FQDN "bash --login -c '/tmp/tests2b.sh mytests $GIT_LIBSTORAGE_COMMIT_ID $GIT_REXRAY_COMMIT_ID' 2>&1" &
