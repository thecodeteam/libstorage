...
scp -i ~/.ssh/${SSH_KEY_FILE} ./config-server.yml $CF_EC2_USER@$EBS_NODE1_FQDN:/tmp/config.yml
scp -i ~/.ssh/${SSH_KEY_FILE} ./tests1.sh $CF_EC2_USER@$EBS_NODE1_FQDN:/tmp
ssh -i ~/.ssh/${SSH_KEY_FILE} $CF_EC2_USER@$EBS_NODE1_FQDN "sudo rm -rf /tmp/finished.txt"
ssh -i ~/.ssh/${SSH_KEY_FILE} $CF_EC2_USER@$EBS_NODE1_FQDN "sudo chmod +x /tmp/tests1.sh"
if [ -f $GIT_LIBSTORAGE_COMMIT_ID ]; then
  scp -i ~/.ssh/${SSH_KEY_FILE} $GIT_LIBSTORAGE_COMMIT_ID $CF_EC2_USER@$EBS_NODE1_FQDN:/tmp
  ssh -i ~/.ssh/${SSH_KEY_FILE} $CF_EC2_USER@$EBS_NODE1_FQDN "sudo chmod +x /tmp/rexray"
fi

scp -i ~/.ssh/${SSH_KEY_FILE} ./config-standalone.yml $CF_EC2_USER@$EBS_NODE2_FQDN:/tmp/config.yml
...
ssh -i ~/.ssh/${SSH_KEY_FILE} $CF_EC2_USER@$EBS_NODE2_FQDN "sudo rm -rf /tmp/finished.txt"
...
if [ -f $GIT_LIBSTORAGE_COMMIT_ID ]; then
  scp -i ~/.ssh/${SSH_KEY_FILE} $GIT_LIBSTORAGE_COMMIT_ID $CF_EC2_USER@$EBS_NODE2_FQDN:/tmp
  ssh -i ~/.ssh/${SSH_KEY_FILE} $CF_EC2_USER@$EBS_NODE2_FQDN "sudo chmod +x /tmp/rexray"
fi

scp -i ~/.ssh/${SSH_KEY_FILE} ./config-client.yml $CF_EC2_USER@$EBS_NODE3_FQDN:/tmp/config.yml
scp -i ~/.ssh/${SSH_KEY_FILE} ./tests3.sh $CF_EC2_USER@$EBS_NODE3_FQDN:/tmp
ssh -i ~/.ssh/${SSH_KEY_FILE} $CF_EC2_USER@$EBS_NODE3_FQDN "sudo rm -rf /tmp/finished.txt"
ssh -i ~/.ssh/${SSH_KEY_FILE} $CF_EC2_USER@$EBS_NODE3_FQDN "sudo chmod +x /tmp/tests3.sh"
if [ -f $GIT_LIBSTORAGE_COMMIT_ID ]; then
  scp -i ~/.ssh/${SSH_KEY_FILE} $GIT_LIBSTORAGE_COMMIT_ID $CF_EC2_USER@$EBS_NODE3_FQDN:/tmp
...
ssh -i ~/.ssh/${SSH_KEY_FILE} $CF_EC2_USER@$EBS_NODE1_FQDN "bash --login -c '/tmp/tests1.sh $CF_STACK_NAME $GIT_LIBSTORAGE_COMMIT_ID $GIT_REXRAY_COMMIT_ID' 2>&1" &
ssh -i ~/.ssh/${SSH_KEY_FILE} $CF_EC2_USER@$EBS_NODE2_FQDN "bash --login -c '/tmp/tests2a.sh $CF_STACK_NAME $GIT_LIBSTORAGE_COMMIT_ID $GIT_REXRAY_COMMIT_ID' 2>&1" &
ssh -i ~/.ssh/${SSH_KEY_FILE} $CF_EC2_USER@$EBS_NODE3_FQDN "bash --login -c '/tmp/tests3.sh $CF_STACK_NAME $GIT_LIBSTORAGE_COMMIT_ID $GIT_REXRAY_COMMIT_ID' 2>&1" &
...
ssh -i ~/.ssh/${SSH_KEY_FILE} $CF_EC2_USER@$EBS_NODE2_FQDN "bash --login -c '/tmp/tests2b.sh $CF_STACK_NAME $GIT_LIBSTORAGE_COMMIT_ID $GIT_REXRAY_COMMIT_ID' 2>&1" &
